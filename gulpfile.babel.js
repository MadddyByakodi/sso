...
gulp.task('copy:extras', () => gulp
  .src([
    ...
    `${clientPath}/extension.html`,
    `${clientPath}/robots.txt`,
    `${clientPath}/.htaccess`,
  ], { dot: true })
  .pipe(gulp.dest(`${paths.dist}/${clientPath}`)));

gulp.task('copy:sw.js', () => gulp
  .src(`${clientPath}/sw.js`, { dot: true })
  .pipe(plugins.babel({
    presets: ['es2015'],
  }))
  .on('error', err => debug('Babel error %s', err.message))
  .pipe(plugins.uglify())
  .pipe(gulp.dest(`${paths.dist}/${clientPath}`)));
...
gulp.task('copy:fonts:dev', () => gulp
  .src('node_modules/{bootstrap,font-awesome,npm-font-source-sans-pro}/fonts/**')
  .pipe(flatten())
